...
以前報告された募集画面の初期表示時の`invalid_recruitment_date`は、ISO内部値と`Asia/Tokyo`固定化で解消し、自動テストで確認済みです。募集画面の起動から公開・応募・通知遷移・応募取消までのiOS実機全通し確認は未完了です。

## 表示言語・利用モードと更新

プロフィールの設定では、表示言語（日本語／英語）と利用モード（`local`／`traveler`）を別々に保存します。表示言語だけを切り替えても、利用モードやログインセッションは変わりません。現在開いている募集、応募履歴、通知、詳細画面は、表示言語の変更を即時に反映します。この操作は既存データを再取得しないため、画面復帰や言語変更によって一覧が勝手に更新されることはありません。

募集・応募・通知の一覧を最新状態にしたい場合は、各画面の明示的な更新操作を使います。通知の遷移は表示文言ではなく通知の構造化IDで決めるため、日本語／英語のどちらで届いた通知も、設定変更後に開けます。

実機確認は [募集カード・マッチングの確認手順](../features/matching.md#9-iphone実機e2eの確認手順) に従います。2026-08-29時点で公開APIの`healthz`と`readyz`、およびiPhone上の表示言語即時切替は確認済みですが、同一手順での募集公開から応募・承認／辞退・取消・ログアウトまでの完走は未確認です。
