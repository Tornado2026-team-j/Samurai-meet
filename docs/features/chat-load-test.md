...
HTTP/3 WebTransport 配送・heartbeat 失効・`token.renew` ローテーション・送信レート制限はバックエンドに実装済みで、
Go の単体テスト（`internal/chat/quic_test.go`など）とサービス統合テストで契約を固定しています。本試験は
**実ネットワーク・実機・複数端末**での挙動とタイミングを確認するもので、自動テストの代替では
ありません。

対象トランスポートは現行サーバーの HTTP/3 WebTransport です。native client moduleが未同梱のため、
実機シナリオは現在BLOCKEDで、native bridge導入後に手動または Maestro / Detox で実施します。

## 2. 前提

- **クライアント**: Development Build（Expo Go 不可。再接続・Secure Storage・バックグラウンド
  遷移が本番相当にならないため）。`EXPO_PUBLIC_API_BASE_URL` を staging に向ける。
- **バックエンド**: staging に本番相当構成でデプロイ。複数インスタンスでは PostgreSQLの
  `LISTEN/NOTIFY` fan-outを有効にする。`APP_ENV`、`CHAT_SEND_BURST` / `CHAT_SEND_REFILL_PER_MINUTE`、
  heartbeat 間隔（現行15s）を本番想定値に固定し記録する。
...
| 6 | マッチ完了 | `POST /matches/{id}/complete` | 両参加者が `closing(chat_not_available)`、以後は履歴・既読のみ可、送信・WebTransport・`transport-token` 不可 |
| 7 | ブロック | 一方が相手をブロック | 送信元は `error(blocked)`→`closing(blocked)`、以後チャットは 404 相当 |
| 8 | 送信レート制限 | 1 端末から上限を超えて連投 | REST は `429 chat_rate_limited`＋`Retry-After`、WebTransportは `error(rate_limited, retry_after_seconds)` で**接続維持**、アプリ再送は同一 `client_message_id`を使い、二重登録なし |
...
実機を多数用意できない場合、WebTransport対応クライアントを Go で多重に張る負荷ハーネスで
サーバー側の容量（goroutine/ヒープ/配送遅延）だけ先行確認できます。ただし実機での
再接続・バックグラウンド・実ネットワークのふるまいは代替できないため、シナリオ 2・11 は
必ず実機で行います。ハーネスを追加する場合は、`backend/internal/chat/quic_test.go` の
token/frame境界と、`backend/internal/integration/chat_fixture_test.go` のDB fixtureを土台にし、
旧WebSocket用のテストヘルパーを再利用しません。

## 6. 関連

- 契約とバックエンド実装: [chat-transport.md](chat-transport.md)
- WebTransportフレーム仕様: [chat-transport.md](chat-transport.md) §イベント配送
- 既存の自動テスト: `TestWebTransportRejectsQueryTokenWithoutEchoingIt` /
  `TestWebTransportOriginBoundary` / `TestQUICRejectsEarlyDataBeforeAnyMutation` /
  `TestChatTranslationRateLimitIsAccountScopedAndSharedInFlight`
