...
現行コードでは募集・検索・関心・承認／辞退のGo APIと、外国人／日本人画面からの募集・募集管理・応募履歴フロー接続コードが存在します。日時入力の初期パース問題はISO内部値／`Asia/Tokyo`固定化と自動テストで解消済みですが、実機での全通し公開・応募・通知遷移は未確認です。
...
- 上記フロント接続、募集管理・応募履歴・応募取り下げ、表示言語に依存しない通知遷移、ログアウト後の履歴ガードはコードと自動テストで確認済み。iOSの日時pickerから公開・応募・通知遷移、ログアウト後の戻る操作までの全通し実機E2Eは未完了とする。
