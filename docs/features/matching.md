...
| 有効期限 | 自動 | 開始時刻まで表示。開始まで6時間未満は注意表示 |
...
`timezone`を省略または空にした募集入力は`Asia/Tokyo`へ正規化し、他のtimezoneは拒否します。`created_at`や`expires_at`などの絶対時刻はUTCで扱います。募集は開始時刻まで公開でき、開始まで6時間未満の場合だけ参加者が集まりにくい可能性をクライアントが注意表示します。ISO内部値、JST固定の日時正規化、期限境界は自動テストで確認済みですが、過去時刻・日跨ぎ・端末日時pickerの実機確認は未完了です。
