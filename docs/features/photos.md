...
- これは暗号文の保管・参加者認可までのバックエンド契約であり、現行クライアントの添付鍵共有・送受信UIおよび本文キー導出は厳密E2EEの完成条件ではない。厳密E2EEを弱めてサーバー復号・AI検査へ寄せる変更は許可しない。
...
- チャット添付の端末間鍵共有・ローテーション・失効を実装して厳密E2EEとして確定する方法。サーバーAIのModeration／翻訳は、平文または鍵をサーバーへ渡さない限り実行できない。
- 削除要求とバックアップからの削除期限。
