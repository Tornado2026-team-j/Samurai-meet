...
| HTTP 429 | `Retry-After` があればその秒数だけ待ってから1回だけ再送。無ければ再送しない。いずれも下記の最大試行回数・全体期限を超えない。 |
| HTTP 400/401/403/404、WS `invalid_input`/`message_too_large`/`blocked`/`chat_not_available`/`chat_closed`/`forbidden`、Chat Token失効 | **自動再送しない。** 必要ならtoken再取得・再接続・再ログインを先に行い、元の送信はユーザー操作か上位状態機械で再評価する。 |
...
- 4xx・入力不正・認証失効・認可拒否・`blocked`/`chat_not_available`/`chat_closed`では自動再送しない。（フロント側の受入条件）
