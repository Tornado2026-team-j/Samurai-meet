...
- Key-B はサーバー側の秘密分割要素として管理し、KMS またはシークレット管理基盤で保護する。
- Key-B は Google 認証と Passkey 認証が完了し、かつ DB 上で失効していないセッションにのみ返し、不要な永続保存を避ける。
- クライアントは Key-A と Key-B を組み合わせ、HKDF でデータ暗号鍵を導出する。
- 画像・暗号化対象データは AES-256-GCM 等で暗号化し、nonce の再利用を禁止する。
- 暗号方式、鍵長、鍵ローテーション、Key-B の配布条件は、実装前にセキュリティレビューで確定する。
...
- Key-B を誰がどの条件で発行するか、KMS とアプリサーバーの信頼境界を設計レビューで確定する。
...
| Q-006 | Key-B | 生成、保管、配布条件、ローテーション、KMS の採用 |
