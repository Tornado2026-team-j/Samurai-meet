...
- チャットは暗号文のREST送信・履歴取得・既読更新・短命transport token発行に加え、WebSocketリアルタイム配送（`internal/chat/websocket.go`、単一APIインスタンス前提のプロセス内ハブ、統合テスト `TestChatWebSocketDelivery`）と、チャット写真の暗号文BLOB添付（`internal/chat/attachment.go`、`0028_chat_attachments.sql`、`POST/GET /api/v1/chats/{id}/attachments`、サーバーは鍵を持たない）を実装済み。QUIC／WebTransport配送、複数インスタンス対応の `LISTEN/NOTIFY` fan-out、`token_seq` ローテーションは未実装。チャット画面のフロント接続は別途。
...
（解消済み）`POST /api/v1/chats/{id}/transport-token` のHTTP handler既定値とサービス受理値の不整合は修正済み。現在はhandler既定値・サービス受理値ともに `websocket`（`webtransport` も受理）で一致する。QUIC配送は引き続き実装済みとは扱わない。
