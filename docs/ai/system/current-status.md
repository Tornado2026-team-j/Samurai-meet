...
- チャットは暗号文のREST送信・履歴取得・既読更新・短命transport token発行に加え、WebSocketによるリアルタイム配送（マルチデバイス整合、送信レート制限、heartbeat失効、接続中のChat Tokenローテーション、複数インスタンス向け`LISTEN/NOTIFY` fan-out）をバックエンド実装済み。QUIC／WebTransportとチャット画面は未実装。
