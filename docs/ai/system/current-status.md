...
- チャットは暗号文のREST送信・履歴取得・既読更新・短命transport token発行に加え、HTTP/3 WebTransport（`internal/chat/quic_server.go`、`ENABLE_CHAT_WEBTRANSPORT` で opt-in、TLS 証明書と UDP アドレスを要求）によるリアルタイム配送（マルチデバイス整合、送信レート制限、接続中のChat Tokenローテーション、15秒間隔 watchdog による失効検知、複数インスタンス向け`LISTEN/NOTIFY` fan-out）と、保持期間スイープ（既定180日・`deleted_at`実配線・暗号文消去・`chat_message_deletions`監査）、チャット写真の暗号文BLOB添付（`internal/chat/attachment.go`、`POST/GET /api/v1/chats/{id}/attachments`、未参照アップロードは約24hでスイープ、サーバーは鍵を持たない）をバックエンド実装済み。既読の `last_message_sequence` はハイウォーターマーク方式（最新liveへクランプ・前進のみ）。transport token は `webtransport` のみ発行（`websocket` は 400、旧 `GET /ws/chats/{id}` は 410）。**閲覧専用化**：`matches.status = completed`、または募集の予定終了時刻（`recruitment_cards.expires_at`）に `CHAT_READONLY_GRACE_HOURS`（既定48時間）を足した時刻を過ぎた `accepted` マッチは、送信・`transport-token`・realtime 接続を `chat_not_available`（HTTP 409）で拒否し、`GET /chats` では `status = "completed"` として返る。履歴・既読は継続可。判定は読み取り時に `internal/chat` の `loadChat` が時刻から導出し、`matches.status` は書き換えない（`internal/chat/readonly.go`、統合テスト `TestChatReadOnlyAfterRecruitmentGrace` / `TestChatReadOnlyClosesLiveWebTransport`）。フロントの再接続・token更新、添付送受信UI・添付鍵共有は未実装。現行本文キーは`chat_id`導出であり厳密E2EEとは扱わない。サーバーAIのチャット本文Moderation（送信前・OpenAI・保持なし）は実装済み、画像Moderation・翻訳は未実装かつ厳密E2EEを維持する限り追加しない。保持日数の最終確定は運用・法務判断。
