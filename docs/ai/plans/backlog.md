...
| P2 | 業務API | profile schema | プロフィール→募集→マッチ→block/report の順で所有権・監査・rate limitを実装。募集カード（最小）・マッチ・blockは実装済み（`backend/internal/matching`）。プロフィール、検索、report、監査ログは未着手 |
