...
| P1 | Chat transport | 業務API | WebSocket配送を実装済み（別audience Chat Token、first-frame認証、20秒heartbeatでセッション/ブロック/マッチ失効＋Chat Token期限、単一インスタンスのin-memoryハブ、fan-out除外はソケット単位でマルチデバイス整合、送信レート制限、接続中の`token.renew`ローテーションと`token_seq`巻き戻し拒否、スレッドstatus列のデッドコード削除）。残: 複数インスタンスの`LISTEN/NOTIFY` fan-out、Expo実機での再接続負荷試験、メッセージ保持期間の実配線 |
