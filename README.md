...
| 募集・マッチング | Go API、外国人/日本人の検索・募集・応募・承認/辞退導線を接続。現在の作業ブランチでは募集画面の初期日時パース失敗が報告され、iOS実機の動作確認は未完了 |
...
DBの期限や監査用の絶対時刻はUTCのRFC3339で保存・返却しますが、募集の日付・時刻として扱う壁時計は常にJSTです。現在のコードではこの契約を実装中で、募集画面のiOS初期表示から公開までの実機確認が残っています。
