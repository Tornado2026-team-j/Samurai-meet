...
- 募集画面はAPI接続コードがあるが、iOS初期表示で`invalid_recruitment_date`が発生した報告があり、日時選択から公開までの実機E2Eを再確認する。
...
3. **残りの業務API・フロント接続**
   ...
   - 評価、チャットのQUICリアルタイム配送、チャット内写真送信は未実装。通知一覧・未読管理と応募／承認／辞退／チャット送信通知は実装済み。
