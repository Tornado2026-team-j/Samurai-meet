...
export async function createKeyMaterial(randomBytes: RandomBytes = expoRandomBytes): Promise<KeyMaterial> {
  ...
  const recoveryDerivedKey = hkdf(sha256, recoveryBytes, salt, RECOVERY_INFO, KEY_BYTES);
  const encryptedKeyA = gcm(recoveryDerivedKey, nonce).encrypt(keyA);
  return {
    keyA,
    recoveryKey,
    envelope: {
...
export function recoverKeyA(recoveryKey: string, envelope: KeyEnvelope): Uint8Array {
  if (envelope.key_version !== KEY_VERSION || envelope.kdf_params.algorithm !== 'HKDF-SHA256') {
    throw new Error('対応していないKey-A envelopeです');
  }
  const recoveryBytes = fromBase64URL(recoveryKey);
  if (recoveryBytes.length !== KEY_BYTES) throw new Error('Recovery Keyの長さが不正です');
  ...
  if (salt.length !== SALT_BYTES || nonce.length !== NONCE_BYTES || info.length === 0) {
    throw new Error('Key-A envelopeのパラメータが不正です');
  }
  const recoveryDerivedKey = hkdf(sha256, recoveryBytes, salt, info, KEY_BYTES);
  return gcm(recoveryDerivedKey, nonce).decrypt(encryptedKeyA);
}
...
export function fromBase64URL(value: string): Uint8Array {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) throw new Error('Base64URL値が不正です');
