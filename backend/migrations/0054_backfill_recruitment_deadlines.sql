-- Align already-open recruitment deadlines with the current rule: cards are
-- visible until 24 hours before their JST start time. Then migrate the legacy
-- TEXT expiry field to TIMESTAMPTZ so searches keep index-friendly timestamp
-- comparisons without relying on a per-row cast.
WITH parsed AS (
    SELECT
        id,
...
ALTER TABLE recruitment_cards
    ADD CONSTRAINT recruitment_cards_expires_at_rfc3339_check
    CHECK (expires_at ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}([.][0-9]+)?(Z|[+-][0-9]{2}:[0-9]{2})$')
    NOT VALID;

ALTER TABLE recruitment_cards
    VALIDATE CONSTRAINT recruitment_cards_expires_at_rfc3339_check;

ALTER TABLE recruitment_cards
    DROP CONSTRAINT recruitment_cards_expires_at_rfc3339_check;
