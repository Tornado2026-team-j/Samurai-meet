...
- `0028_chat_attachments.sql` はチャット写真の暗号文メタデータ`chat_attachments`を追加する。画像本体はDBに保存せず暗号文BLOBストレージへ置き、平文・EXIF・暗号鍵は保存しない。`message_id`が`NULL`の未参照アップロードは起動時スイープで削除する。`--`コメント内に`;`を書かない（runnerは`;`で文を分割する）。
