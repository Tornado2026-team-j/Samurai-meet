...
画面にはGoogle OAuth、Google直後の`pre_auth_token`、Passkey登録・認証・一覧・解除、Refresh、ログアウトを用意しています。Google交換後は通常sessionを発行せず、Passkey成功後だけ通常sessionになります。Expo Goからこの画面を開く場合は、URLのfragmentで渡されたpre-authを使ってPasskeyを完了し、`session-handoff`でアプリへ戻ります。ブラウザのWebAuthnテストは、サーバーの `WEBAUTHN_RP_ID` と `WEBAUTHN_RP_ORIGIN` を画面を開いたOriginに合わせてから実行してください。本番ドメインで行う場合は `samurai-meet.disnana.com` / `https://samurai-meet.disnana.com` を使用し、OAuthの戻り先は `/auth/complete` です。
