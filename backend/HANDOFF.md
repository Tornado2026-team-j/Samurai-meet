...
- チャット送信前Moderationは通常`OPENAI_API_KEY`を使い、キー未設定・provider障害時はfail-closedです。`CHAT_MODERATION_DEV_FREE_MODE=true`を明示した場合だけ、APP_ENVにかかわらず外部送信をしない保守的なローカル判定へ切り替え、起動時警告を出します。これは実データを使わない一時確認例外で、通常の本番運用前に無効化・キー設定が必要です。
...
- migration本体と適用済み履歴は、現行のmigration一覧・checksum検証に従う。適用済みmigrationを編集・削除して回避しない。
