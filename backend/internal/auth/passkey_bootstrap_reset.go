...
func (s *PasskeyBootstrapService) ResetCeremony(ctx context.Context, token string, scope PasskeyBootstrapScope, userID, sessionID, ceremonyToken string, now time.Time) error {
	...
	result, err := tx.ExecContext(ctx, `UPDATE auth_challenges SET used_at=$1 WHERE token_hash=$2 AND type=$3 AND used_at IS NULL`, now.UTC().Format(time.RFC3339Nano), hashOAuthState(ceremonyToken), string(scope))
	if err != nil {
		return err
	}
	...
	result, err = tx.ExecContext(ctx, `UPDATE passkey_bootstraps SET ceremony_token_hash=NULL WHERE token_hash=$1 AND ceremony_token_hash=$2 AND used_at IS NULL`, hashOAuthState(token), hashOAuthState(ceremonyToken))
	if err != nil {
		return err
	}
	if affected, rowsErr := result.RowsAffected(); rowsErr != nil || affected != 1 {
